...
GLfloat square[8][2];
void prepare_square(float square_size) {
	square[0][0] = -square_size; square[0][1] = square_size;
	square[1][0] = square_size; square[1][1] = square_size;
	square[2][0] = -square_size; square[2][1] = -square_size;
	square[3][0] = square_size; square[3][1] = -square_size;

	square[4][1] = -square_size; square[4][0] = square_size;
	square[5][1] = square_size; square[5][0] = square_size;
	square[6][1] = -square_size; square[6][0] = -square_size;
	square[7][1] = square_size; square[7][0] = -square_size;
	glGenBuffers(1, &VBO_axes);

	glBindBuffer(GL_ARRAY_BUFFER, VBO_axes);
	glBufferData(GL_ARRAY_BUFFER, sizeof(square), axes, GL_STATIC_DRAW);
...
void update_square(float square_size) {
	square_size = 300;
	square[0][0] = -square_size; square[0][1] = square_size;
	square[1][0] = square_size; square[1][1] = square_size;
	square[2][0] = -square_size; square[2][1] = -square_size;
...
void draw_house_body() {
	glBindVertexArray(VAO_house);
	glUniform3fv(loc_primitive_color, 1, house_color[HOUSE_BODY]);
	glDrawArrays(GL_TRIANGLE_FAN, 3, 4);
	glBindVertexArray(0);
}
void draw_house_chimney() {
	glBindVertexArray(VAO_house);
	glUniform3fv(loc_primitive_color, 1, house_color[HOUSE_CHIMNEY]);
	glDrawArrays(GL_TRIANGLE_FAN, 7, 4);
	glBindVertexArray(0);
}
void draw_house_door() {
	glBindVertexArray(VAO_house);
	glUniform3fv(loc_primitive_color, 1, house_color[HOUSE_DOOR]);
	glDrawArrays(GL_TRIANGLE_FAN, 11, 4);
	glBindVertexArray(0);
...
GLfloat sword_hit_box[4][2] = { {-6,20},{-6,-10},{6,20},{6,10} };
...
#define HUMAN_BODY 0
#define HUMAAN_HEAD 1



GLfloat human_body[4][2] = { {-1.0,1.8},{1.0,1.8},{1.0,0},{-1.0,0} };
GLfloat human_head[360][2] = { 0.0 };
GLfloat human_right_arm[4][2] = { {-1.0,1.8},{-1.5,1.5},{-1.5,0},{-1.0,0} };
GLfloat human_left_arm1[4][2] = { {1.0,1.8},{2.0,1.0},{2.0,0.5},{1.0,1.0} };
GLfloat human_left_arm2[4][2] = { { 2.0,1.0 },{ 2.0,0.5 },{ 3.0,1.0 },{ 3.0,1.5 } };
GLfloat human_left_leg[4][2] = { {-1.0,0},{-1,-1.5},{-0.4,-1.5},{-0.3,0} };
GLfloat human_right_leg[4][2] = { { 1.0,0 },{ 1,-1.5 },{ 0.4,-1.5 },{ 0.3,0 } };
GLfloat human_left_foot[4][2] = { { -0.4,-1.5 },{ -1.5,-1.5 },{ -1.5,-2.0 },{ -0.4,-2.0 } };
GLfloat human_right_foot[4][2] = { {0.4,-1.5},{1.5,-1.5},{1.5,-2.0},{0.4,-2.0} };

GLfloat human_left_arm2_2[4][2] = { { 2.0,1.0 },{ 2.0,0.5 },{ 2.8,1.6 } ,{ 2.4,2.0 } };

GLuint VBO_human, VAO_human;

GLfloat human_color[7][3] = {
	{ 29 / 255.0f, 161 / 255.0f, 227 / 255.0f },
{ 251 / 255.0f, 203 / 255.0f, 151 / 255.0f },
{ 155 / 255.0f, 155 / 255.0f, 155 / 255.0f },
{ 155 / 255.0f, 155 / 255.0f, 155 / 255.0f },
{ 0 / 255.0f, 0 / 255.0f, 0 / 255.0f },
{ 139 / 255.0f, 69 / 255.0f, 19 / 255.0f },
...
void prepare_human() {
	GLsizeiptr buffer_size = sizeof(human_body) + sizeof(human_head) + sizeof(human_right_arm) + sizeof(human_left_arm1) + sizeof(human_left_arm2) + sizeof(human_right_leg) + sizeof(human_left_leg) + sizeof(human_left_foot) + sizeof(human_right_foot) + sizeof(human_left_arm2_2);
	for (int i = 0; i < 360; i++) {
		human_head[i][0] = 0.7 * cosf(float(i) * TO_RADIAN);
		human_head[i][1] = 2.5 + 0.7 * sinf(float(i) * TO_RADIAN);
	}
	// Initialize vertex buffer object.
	...
	glBufferData(GL_ARRAY_BUFFER, buffer_size, NULL, GL_STATIC_DRAW); // allocate buffer object memory
	int bufsize = 0;
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_body), human_body); bufsize += sizeof(human_body);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_head), human_head); bufsize += sizeof(human_head);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_right_arm), human_right_arm); bufsize += sizeof(human_right_arm);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_left_arm1), human_left_arm1); bufsize += sizeof(human_left_arm1);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_left_arm2), human_left_arm2); bufsize += sizeof(human_left_arm2);



	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_right_leg), (human_right_leg)); bufsize += sizeof(human_right_leg);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_left_leg), (human_left_leg)); bufsize += sizeof(human_left_leg);

	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_left_foot), (human_left_foot)); bufsize += sizeof(human_left_foot);
	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_right_foot), (human_right_foot)); bufsize += sizeof(human_right_foot);

	glBufferSubData(GL_ARRAY_BUFFER, bufsize, sizeof(human_left_arm2_2), (human_left_arm2_2)); bufsize += sizeof(human_left_arm2_2);
...
void draw_human() {
	...
	glUniform3fv(loc_primitive_color, 1, human_color[HUMAN_BODY]);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4);

	glUniform3fv(loc_primitive_color, 1, human_color[HUMAAN_HEAD]);
	glDrawArrays(GL_TRIANGLE_FAN, 4, 360);


	glUniform3fv(loc_primitive_color, 1, human_color[SWORD_HEAD]);
