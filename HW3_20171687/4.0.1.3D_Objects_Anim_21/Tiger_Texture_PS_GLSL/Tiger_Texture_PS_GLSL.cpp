...
void display(void) {
	...
	*/
	...
	ModelViewMatrix = glm::rotate(ViewMatrix, -bike.angle * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
	ModelViewMatrix = glm::translate(ModelViewMatrix, bike.pos);
	ModelViewMatrix = glm::rotate(ModelViewMatrix, (bike.up_angle-90) * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
...
void timer_scene(int value) {
	...
	ben_timestamp = (ben_timestamp + 1) % UINT_MAX;
	spider_timestamp = (spider_timestamp + 1) % UINT_MAX;

	cur_frame_tiger = tiger_timestamp % N_TIGER_FRAMES;
	cur_frame_ben = ben_timestamp % N_BEN_FRAMES;
	cur_frame_wolf= wolf_timestamp % N_WOLF_FRAMES;
	cur_frame_spider = spider_timestamp % N_SPIDER_FRAMES;
...
void main(int argc, char *argv[]) {
	...
	char messages[N_MESSAGE_LINES][256] = { 
		...
		"    -  '1', '2', '3', '4', '5' : 카메라 조작",
		"    - 'x', 'y', 'z' : 5번 카메라 움직일 때 기준 축 변경",
		"    - 'w', 's', 'a', 'd' : w, s로 기준축에 대해 5번 카메라 움직이기, a,d로 5번 카메라 회전하기",
		"    - 'key up', 'key down', 'key left', 'key right' : 1번 카메라 시점 변경 ",
		"    - 'u', 'i', 'o', 'p'  : 동적인 물체 움직이기",
		"    - 'ESC' : 프로그램 종료"
	};
